...
def embed_files():
    try:
        ...
        # Process files and save to Milvus
        model_name = request.form.get('model_name')
        chunk_size = int(request.form.get('chunk_size'))
        overlap = int(request.form.get('overlap'))
        generate_embeddings.embedding_files_to_milvus(file_paths, model_name, chunk_size, overlap)
...
def handle_query():
    ...
    def generate():
        try:
            answer_query_with_streaming.initialize_settings(language_model, embedding_model)
            stream_generator = answer_query_with_streaming.build_and_query_index_in_milvus(query, embedding_model, top_k)
            for chunk in stream_generator:
                yield chunk
        except Exception as e:
            yield f"Error processing query: {str(e)}"
